...
if(isset($_GET['vid'])) {
	if(!is_numeric($_GET['vid'])) {
		echo "Vrienden id is meer dan alleen nummers.<br />Dit is niet toegestaan.<br /><a href='javascript:history.go(-1)'>Ga terug</a>";
		...
		if(mysql_num_rows($check_bestaan_vriend) >= 1) {
			echo "Deze gebruiker is al jou vriend.<br />Het kan zijn dat hij jou nog moet accepteren als vriend.<br /><a href='javascript:history.go(-1)'>Ga terug</a>";
				...
				if(isset($_GET['a'])) {
					if($_GET['a'] == "verwijderen") {
						...
						if(mysql_num_rows($check_van_jou) != 1) {
							...
							echo "De vriend is verwijderd uit je vriendenlijst.<br /><a href='javascript:history.go(-1)'>Ga terug</a>";
						}
